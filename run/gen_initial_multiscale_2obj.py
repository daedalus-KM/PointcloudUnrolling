...
def discard_outliers(disp_, step, thresh=15, check = False, temp = False, h_range = (0, 0), w_range = (0, 0)):
    ...
    for it in range(len(ii)):
        ...
        vv = []
        for j in range(max(0, idx1-step), min(W-1, idx1+step+1)):
...
def gen_initial_multiscale(tof, h1, KK=[1,3,7,13], attack=None, trailing=None, use_cuda=False):
    if len(h1.shape) != 1:
